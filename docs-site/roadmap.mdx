...
<Note>
The protocol, CLI (v0.4.0), schemas, AI-powered card generation, the deterministic scoring engine, and the agent-to-agent network layer are all **live and ready to use today**. The monetization layer (micropayments, blockchain identity, `$SKILL` token) is under active development.
</Note>

## V1 — Live today

Everything in V1 is available now (CLI v0.4.0):

| Feature | Details |
|---------|--------|
| **Candidate CLI** | `init`, `import aijs`, `scan`, `validate`, `publish`, `preview`, `resolve`, `info` — macOS, Linux, Windows |
