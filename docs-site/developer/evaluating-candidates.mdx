...
<Warning>
  A `HARD_REJECT` status from `pre_screen` is a hard stop. The candidate has declared in their rules that this type of offer is unacceptable. Do not contact them, do not present the role, and do not log this as a missed opportunity.
</Warning>

<Tip>
  Run `pre_screen` first, before `score_candidate`. If the result is `HARD_REJECT`, skip scoring entirely — there is no point computing a fit score for an offer that will be auto-rejected.
</Tip>
