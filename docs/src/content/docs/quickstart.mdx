...
<Steps>
  ...
  <Step title="Manual CLI Install">
    ...
    <Note>
      ...
      capability set `windows-subset-v1`. It supports only `init`, `init --ai`,
      `validate`, `publish`, `info`, `help`, and `version`; use macOS/Linux or a
      POSIX environment for `scan` and the rest of the full CLI.
    </Note>

    Then, build your card manually using one of these methods:
    <Tabs>
      <TabItem label="AI Scan (POSIX)">
