...
<Note>
  Installation and document extraction run locally. During `scoutica scan`, a remote-capable AI provider may send the full generated prompt and document text to its configured service. Review the provider and endpoint before scanning sensitive files.
</Note>
...
- **Strict validation dependencies** — install explicitly with `python3 -m pip install 'jsonschema[format]' PyYAML`
- **An AI CLI provider (macOS/Linux scan only)** — at least one provider below must be installed for `scoutica scan`
...
Protocol version and CLI implementation version are separate identities.

| Platform | Implementation | Capability set | Supported commands |
|----------|----------------|----------------|--------------------|
| macOS / Linux | POSIX 0.4.0 | Full POSIX surface | All commands in the [CLI reference](/cli/overview) |
| Windows PowerShell 5.1 / PowerShell 7 | PowerShell implementation 0.1.0 | `windows-subset-v1` | `init`, `init --ai`, `validate`, `publish`, `info`, `help`, `version` |
...
| Mistral Vibe | `vibe` | [mistralai/mistral-vibe](https://github.com/mistralai/mistral-vibe) | Interactive CLI provider for focused tasks |
| OpenCode | `opencode` | [opencode-ai/opencode](https://github.com/opencode-ai/opencode) | Open source LLM bridging via bash |
| Ollama | `ollama` | [ollama.com](https://ollama.com) | Local when its configured endpoint is loopback |
...
The Scoutica Protocol is designed to be fully operated by autonomous AI agents (like Claude Code, Cursor, OpenCode, or OpenClaw). The easiest and fastest way to install the CLI and configure the protocol is natively via your agent's terminal. 
...
<Steps>
  <Step title="Run the installer">
    ...
    3. **Schemas download:** Pulls schemas into `~/.scoutica/schemas` for blazing fast offline validation.
    4. **Templates download:** Pulls rules and markdown templates used for generation.
    5. **Helper download:** Installs the helpers declared for the platform. The full POSIX install includes scoring, import, validation, scan, and safe-fetch helpers; Windows installs the resources declared for `windows-subset-v1`.
    6. **Path registration:** Appends `~/.scoutica/bin` to your `PATH`.
  </Step>
...
## Manual macOS/Linux Installation
...
<Steps>
  ...
  <Step title="Copy core files">
    ...
    # Python helpers the CLI shells out to (evaluate / import / validate) — MUST sit beside the binary
    cp tools/scoring.py tools/import_aijs.py tools/validate_card.py ~/.scoutica/bin/
