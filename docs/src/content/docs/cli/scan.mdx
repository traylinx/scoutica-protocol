...
<Warning>
`scan` requires a configured AI provider in your PATH (for example, `gemini`, `claude`, or `ollama`). A remote-capable provider may receive the full generated prompt and document text. `--clipboard` places that same sensitive payload on your system clipboard for user-controlled transfer.
</Warning>
