...
The complete envelope is checked against the bundled trusted `message.schema.json`, including URI and date-time formats, before the first write. Each artifact is written atomically and symlinked destinations are refused.
