...
<Note>
Protocol 0.4.0, the POSIX implementation 0.4.0, schemas, AI-powered card generation, the deterministic scoring engine, and the agent-to-agent network layer are all **live and ready to use today**. Windows ships PowerShell implementation 0.1.0 under capability set `windows-subset-v1`; full Windows parity is parked. The monetization layer (micropayments, blockchain identity, `$SKILL` token) is under active development.
</Note>

## V1 — Live today

Everything below is available now in the POSIX implementation 0.4.0 unless a platform row says otherwise:
...
When an employer's agent unlocks Zone 2 (full profile), a ~$0.05 micro-fee is paid directly to the candidate. This fee isn't just about monetization — it acts as a hard economic barrier against recruiter spam and mass data scraping. If it costs money to view a profile, recruiters only unlock candidates they are serious about matching with. V1 uses Stripe credits; V2 moves to on-chain micro-fees.
