...
| `scan_runtime/pty_run.py` | scan consent tests | Runs leader or nonleader CLI paths in a PTY, feeds yes/no/EOF, and verifies foreground-group restoration. |
| `scan_runtime/pty_signal_run.py` | scan lifecycle tests | Exercises direct-PID and terminal Ctrl-C delivery after an interactive prompt, including a non-group-leader CLI and foreground restoration. |
| `scan_runtime/signal_run.py` | scan lifecycle tests | Signals the scan parent only after the provider double reports readiness. |
| `scan_runtime/nonleader_signal_run.py` | scan lifecycle tests | Signals a CLI that shares its supervisor's process group, then verifies the isolated test session drains. |
| `scan_runtime/slow_wc.py` | scan lifecycle tests | Holds a pre-provider `wc` subprocess open to exercise Bash 3.2 signal-tree cleanup. |
| `scan_runtime/startup_delay/sitecustomize.py` | scan lifecycle tests | Delays only the grouped Python helper to make its pre-signal-reset startup window deterministic. |
